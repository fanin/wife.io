...
fse.removeSync('.sublime-gulp.cache');
...
function addBuildTargets() {
  ...
  var RunStopScript = util.format(
      ...
      '                    || cd %s; npm restart' + os.EOL +
      '        ;;' + os.EOL +
      '    stop)' + os.EOL +
      '        echo Stopping %s...; ' + os.EOL +
      '        cd %s; npm stop' + os.EOL +
      '        ;;' + os.EOL +
      '    *)' + os.EOL +
      'esac' + os.EOL,
  ...
  gulp.task('clean', function(cb) {
    fse.remove(global.BUILD_PATH, cb);
  });

  gulp.task('distclean', function(cb) {
    fse.removeSync('.build-device');
    fse.removeSync('.build-runstop.sh');
    fse.remove(global.BUILD_PATH, cb);
  });
}
