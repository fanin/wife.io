...
var Markdown = React.createClass({
    ...
    render: function() {
        return (
            <span dangerouslySetInnerHTML={{__html: marked(this.props.string, {sanitize: true})}}
    ...
    render: function() {
        ...
        var apiMethodTable = this.props.api ? this.props.api.apimethods.map(function(method) {
            ...
            var paramRows = method.apiparam ? method.apiparam.map(function(param) {
                return (
                    <tr>
                        <td colSpan={4} className="center aligned"><code>{param.name}</code></td>
                        <td colSpan={4} className="center aligned">{param.type}</td>
                        <td colSpan={12}><Markdown string={param.description} /></td>
                    </tr>
                );
            }) : [];

            var postDataRows = method.apipostdata ? method.apipostdata.map(function(postdata) {
                return (
                    <tr>
                        <td colSpan={4} className="center aligned"><code>{postdata.name}</code></td>
            ...
            var responseRows = method.apireturn ? method.apireturn.map(function(response) {
                return (
                    <tr>
                        ...
                        <td colSpan={4} className={"center aligned" + (response.statustext ? "" : " disabled")}>{response.statustext ? response.statustext : "Default"}</td>
                        <td colSpan={3} className={"center aligned" + (response.name ? "" : " disabled")}><code>{response.name ? response.name : "N/A"}</code></td>
                        <td colSpan={2} className={"center aligned" + (response.type ? "" : " disabled")}>{response.type ? response.type : "N/A"}</td>
                    ...
                    <table id={"table-" + method.apimethod.name} className={tableClass}>
                        <tbody>
                            ...
                            <tr style={{display: method.apiparam ?  "table-row" : "none"}}>
                                <td className="active center aligned" rowSpan={method.apiparam ? method.apiparam.length + 1 : 1}>Parameters</td>
                            ...
                            <tr style={{display: method.apipostdata ?  "table-row" : "none"}}>
                                <td className="active center aligned" rowSpan={method.apipostdata ? method.apipostdata.length + 1 : 1}>Post Data</td>
                                <td className="active center aligned" colSpan={4}>Name</td>
                                <td className="active center aligned" colSpan={4}>Type</td>
                                <td className="active" colSpan={12}>Description</td>
                            </tr>
                            ...
                            <tr style={{display: method.apioption ?  "table-row" : "none"}}>
                                <td className="active center aligned" rowSpan={method.apioption ? method.apioption.length + 1 : 1}>Options</td>
                                <td className="active center aligned" colSpan={4}>Name</td>
                                <td className="active center aligned" colSpan={4}>Type</td>
                                <td className="active" colSpan={12}>Description</td>
                            </tr>
                            ...
                            <tr style={{display: method.apireturn ?  "table-row" : "none"}}>
                                <td className="active center aligned" rowSpan={method.apireturn ? method.apireturn.length + 1 : 1}>Responses</td>
...
var AppMainView = React.createClass({
        ...
        FSAPI.readFile("restapi.json::1", { encoding: "ascii" }, {
            onSuccess: function(data) {
                if (data) {
                    ...
                    var defmethod = (apidata[0].apimethods && apidata[0].apimethods.length > 0)
                                    ? apidata[0].apimethods[0].apimethod.name : "";

                    this.setState({ apiData: apidata, apiClass: defclass, apiMethod: defmethod });
    ...
    shouldComponentUpdate: function (nextProps, nextState) {
        if (nextState.apiClass !== this.state.apiClass || nextState.apiMethod !== this.state.apiMethod)
            return true;
        else
            return false;
    },
    ...
    componentDidUpdate: function (prevProps, prevState) {
        ...
        if (this.currentApi && this.currentApi.apiclass === this.state.apiClass) {
            this.currentApi.apimethods.map(function(method) {
                ...
                offsets.push({ method: method.apimethod.name, offset: top, height: height });
            });
        }
            ...
            return (
                <div className="api-menu">
                    ...
                    <div className="ui inverted secondary vertical pointing menu"
                             style={{ display: this.state.apiClass === api.apiclass ? "block" : "none"}}>
                        {apiMethodItems}
                    </div>
                </div>
            );
    ...
    handleScroll: function(event) {
        ...
        for (var i in this.anchorOffsets) {
            if (scrollTop < this.anchorOffsets[i].offset + this.anchorOffsets[i].height * 2 / 3) {
