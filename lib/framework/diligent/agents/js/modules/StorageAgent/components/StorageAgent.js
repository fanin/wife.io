...
var StorageAgent = assign({}, EventEmitter.prototype, {
    ...
    _onStorageChange: function(change) {
        switch (change.actionType) {
            ...
            case StorageConstants.STORAGE_SET_DISK_INUSE_FAIL:
                StorageAgent.emit('storage.setDiskInUseDidFail', { disk: change.disk, error: change.error });
                break;
    ...
    attach: function(component) {
        if (component.storageListDidReceive)
            StorageAgent.on('storage.listDidReceive', component.storageListDidReceive);
        ...
        if (component.storageSetInUseDidFail)
            StorageAgent.on('storage.setDiskInUseDidFail', component.storageSetInUseDidFail);
    ...
    detach: function(component) {
        if (component.storageListDidReceive)
            StorageAgent.off('storage.listDidReceive', component.storageListDidReceive);
        if (component.storageDidMount)
        ...
        if (component.storageSetInUseDidFail)
            StorageAgent.off('storage.setDiskInUseDidFail', component.storageSetInUseDidFail);
        if (component.storageInUseDidChange)
