...
var DiligentClient = assign({}, EventEmitter.prototype, {
    ...
    init: function() {
        ...
        function registerBaseListeners() {
             ...
             */
            self.ioClient.on(self.wsapi[0].Base.GetManifest.RES, function(manifest) {
                self.manifest = manifest;
                self.emit('diligent.wsapi#loaded');
            });

            self.ioClient.on(self.wsapi[0].Base.GetManifest.ERR, function(error) {
        ...
        function unregisterBaseListeners() {
            ...
            self.ioClient.removeAllListeners(self.wsapi[0].Base.GetManifest.RES);
            self.ioClient.removeAllListeners(self.wsapi[0].Base.GetManifest.ERR);
        }

        self.loadWSwsapi(function(error) {
            if (!error) {
                 ...
                 */
                self.ioClient.emit(
                    self.wsapi[0].Base.GetManifest.REQ,
                    {
