...
var TaskQueue    = require('lib/common/task-queue');
...
var AppManagerClient = assign({}, EventEmitter.prototype, {
    ...
    attach: function(_super) {
        ...
        this.socket.on(this.APISpec.Install.RES, function(status, instid) {
        ...
        this.socket.on(this.APISpec.Install.ERR, function(instid, error) {
            var task = this.taskQueue.getCurrentTask();
            if (!task) return;
        ...
        this.socket.on(this.APISpec.CancelInstall.RES, function(instid) {
            this.emit("app.install#cancelled", { instid: instid });
        }.bind(this));

        this.socket.on(this.APISpec.CancelInstall.ERR, function(instid, error) {
            this.emit("app.install#cancel-error", { instid: instid, error: error });
        }.bind(this));

        this.socket.on(this.APISpec.Uninstall.RES, function(manifest) {
    ...
    install: function(appBundleFile, onProgress) {
        var instid = utils.generateUID();

        this.taskQueue.addTask({
            socket: this.socket,
            instApi: this.APISpec.Install,
            instid: instid,
    ...
    uninstall: function(manifest) {
        this.socket.emit(this.APISpec.Uninstall.REQ, manifest);
    },
    ...
    cancelInstall: function(instid) {
        ...
        if (task) {
            ...
            if (task.context.appReadStream) {
                ...
                this.socket.emit(this.APISpec.CancelInstall.REQ, instid);
                this.taskQueue.finishTask(task);
            }
