...
CoreClient.prototype.registerProtoListeners = function(socket) {
    ...
    socket.on(self.protocol.MinorVersion[0].Extension.Load.Return.Loaded, function(extName, extProto) {
        if (extProto) {
    ...
    socket.on(self.protocol.MinorVersion[0].Extension.Unload.Return.Unloaded, function(extName) {
    ...
    socket.on(self.protocol.MinorVersion[0].Extension.Load.Error, function(extName) {
        self.delegate && self.delegate.protoErrorExtProtoLoad(self);
    });

    socket.on(self.protocol.MinorVersion[0].Extension.Unload.Error, function(extName) {
        self.delegate && self.delegate.protoErrorExtProtoUnload(self);
    });

    /* Dispatch APP protocol response events */
    socket.on(self.protocol.MinorVersion[0].APP.List.Return.List, function(apps) {
        self.delegate && self.delegate.protoResponseAppList(self, apps);
    });

    socket.on(self.protocol.MinorVersion[0].APP.Install.Return.Uploading, function(installationCode) {
        self.delegate && self.delegate.protoResponseAppUploading(self, installationCode);
    });

    socket.on(self.protocol.MinorVersion[0].APP.Install.Return.Installing, function(installationCode) {
        self.delegate && self.delegate.protoResponseAppInstalling(self, installationCode);
    });

    socket.on(self.protocol.MinorVersion[0].APP.Install.Return.Installed, function(installationCode) {
        self.delegate && self.delegate.protoResponseAppInstalled(self, installationCode);
    });

    socket.on(self.protocol.MinorVersion[0].APP.CancelInstall.Return.Cancelled, function(installationCode) {
        self.delegate && self.delegate.protoResponseAppInstallCancelled(self, installationCode);
    });

    socket.on(self.protocol.MinorVersion[0].APP.Uninstall.Return.Uninstalled, function(appInfo) {
        self.delegate && self.delegate.protoResponseAppUninstalled(self, appInfo);
    });

    socket.on(self.protocol.MinorVersion[0].APP.Install.Error, function(error) {
        self.delegate && self.delegate.protoErrorAppInstall(self, error);
    });

    socket.on(self.protocol.MinorVersion[0].APP.Uninstall.Error, function(error) {
        self.delegate && self.delegate.protoErrorAppUninstall(self, error);
    });

    /* Dispatch storage protocol response events */
    socket.on(self.protocol.MinorVersion[0].Storage.GetLocalDisks.Return.Disks, function(disks) {
        self.delegate && self.delegate.protoResponseLocalDisks(self, disks);
    });

    socket.on(self.protocol.MinorVersion[0].Storage.GetLocalDisks.Error, function(error) {
...
CoreClient.prototype.unregisterProtoListeners = function(socket) {
    ...
    socket.removeAllListeners(this.protocol.MinorVersion[0].Extension.Load.Return.Loaded);
    socket.removeAllListeners(this.protocol.MinorVersion[0].Extension.Unload.Return.Unloaded);
    socket.removeAllListeners(this.protocol.MinorVersion[0].Extension.Load.Error);
    socket.removeAllListeners(this.protocol.MinorVersion[0].Extension.Unload.Error);

    /* Remove APP protocol response events */
    socket.removeAllListeners(this.protocol.MinorVersion[0].APP.List.Return.List);
    socket.removeAllListeners(this.protocol.MinorVersion[0].APP.Install.Return.Installing);
    socket.removeAllListeners(this.protocol.MinorVersion[0].APP.Install.Return.Installed);
    socket.removeAllListeners(this.protocol.MinorVersion[0].APP.CancelInstall.Return.Cancelled);
    socket.removeAllListeners(this.protocol.MinorVersion[0].APP.Uninstall.Return.Uninstalled);
    socket.removeAllListeners(this.protocol.MinorVersion[0].APP.Install.Error);
    socket.removeAllListeners(this.protocol.MinorVersion[0].APP.Uninstall.Error);

    /* Remove storage protocol response events */
    socket.removeAllListeners(this.protocol.MinorVersion[0].Storage.GetLocalDisks.Return.Disks);
    socket.removeAllListeners(this.protocol.MinorVersion[0].Storage.GetLocalDisks.Error);
}

/* Extension client APIs */
CoreClient.prototype.extensionLoad = function(extension) {
    this.extension[extension.name] = extension;
    this.ioClient.emit(this.protocol.MinorVersion[0].Extension.Load.Command, extension.name, extension.version);
}

CoreClient.prototype.extensionUnload = function(extension) {
    this.ioClient.emit(this.protocol.MinorVersion[0].Extension.Unload.Command, extension.name, extension.version);
}

/* APP client APIs */
CoreClient.prototype.appGetList = function() {
    this.ioClient.emit(this.protocol.MinorVersion[0].APP.List.Command);
}

CoreClient.prototype.appInstall = function(sioStream, appBundle, onDataFn) {
    try {
        this.appBundleDataStream = sioStream.createStream();
        sioStream(this.ioClient).emit(this.protocol.MinorVersion[0].APP.Install.Command, this.appBundleDataStream);
...
CoreClient.prototype.appUninstall = function(appInfo) {
    this.ioClient.emit(this.protocol.MinorVersion[0].APP.Uninstall.Command, appInfo);
}
...
CoreClient.prototype.appCancelInstall = function(installationCode) {
    if (this.blobStream && this.appBundleDataStream) {
        ...
        this.ioClient.emit(this.protocol.MinorVersion[0].APP.CancelInstall.Command, installationCode);
...
CoreClient.prototype.storageGetLocalDisks = function() {
    this.ioClient.emit(this.protocol.MinorVersion[0].Storage.GetLocalDisks.Command);
}
