...
CoreClient.prototype.initiate = function() {
    ...
    function loadWSAPISpec() {
        var result = false;
        var apiSpec = 'wsapi-spec-v' + self.version;
        ...
        $.ajax({
            ...
            success: function(data) {
                try {
                    ...
                    self.event.trigger('core.wsapi#error', error);
                }
            }
        });
    ...
    function registerProtoListeners() {
        ...
        self.ioClient.on(self.apiSpec[0].Base.GetInfo.RES, function(appInfo) {
            ...
            self.event.trigger('core.wsapi#loaded');
        });
    ...
    else alert('CoreClient load api specification error');
}
