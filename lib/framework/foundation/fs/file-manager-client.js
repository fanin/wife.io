...
FileManagerClient.prototype.register = function(_super) {
    ...
    this.addApiCallback = function(api, index, callback) {
        ...
        this.api[api][index] = this.api[api][index] || [];
        this.api[api][index].push(callback);
    };
    ...
    this.socket.on(this.protoFS.Exist.RES, function(path, exist) {
        self.forEachApiCallback('Exist', path, function(apiCallback) {
            apiCallback(path, exist, null);
        });
    });
    this.socket.on(this.protoFS.Exist.ERR, function(path, error) {
        self.forEachApiCallback('Exist', path, function(apiCallback) {
            apiCallback(path, false, error);
        });
    });
...
FileManagerClient.prototype.readFile = function(path, func) {
    this.addApiCallback('ReadFile', path, func);
    this.socket.emit(this.protoFS.ReadFile.REQ, path);
};
