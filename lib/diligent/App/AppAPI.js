...
var strext = require('utils/common/string-ext');
...
module.exports = {
    list: function(callbacks) {
        ...
        apiutil.get('/api/v1/apps', {
            ...
            error: function(xhr) {
                callbacks.onError && callbacks.onError({ code: xhr.status, message: 'Unable to get app list.' });
            }
        });
    },
    ...
    install: function(appPkgFile, callbacks) {
        ...
        apiutil.upload('/api/v1/apps/install', appPkgFile, {
            xhrFields: {
                clientInstallationID: strext.uniqueString()
            }
        }, {
            ...
            error: function(xhr) {
                callbacks.onError && callbacks.onError({ code: xhr.status, message: xhr.responseText || xhr.statusText }, xhr);
            },
    ...
    getInstallStatus: function(instid, callbacks) {
        ...
        apiutil.get('/api/v1/apps/install/' + instid, {
            success: function(xhr) {
                callbacks.onSuccess && callbacks.onSuccess({ code: xhr.status, message: xhr.responseText || xhr.statusText });
            },
            error: function(xhr) {
                callbacks.onError && callbacks.onError({ code: xhr.status, message: xhr.responseText || xhr.statusText }, xhr);
            }
    ...
    uninstall: function(manifest, keepUserData, callbacks) {
        callbacks = callbacks || {};
        apiutil.delete('/api/v1/apps/' + manifest.identifier + '?keepUserData=' + (keepUserData ? 1 : 0), {
